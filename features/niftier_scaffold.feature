Feature: Niftier Scaffold Generator
  ...
  Scenario: Generate scaffold for simple resource
    ...
    And I should successfully run "rails g niftier:scaffold Project -f"
    Then I should successfully run "rake test"

  Scenario: Generate scaffold with rspec tests
    Given a new Rails app
    When I run "rails g niftier:scaffold Project name:string --rspec"
    Then I should see the following files
      | spec/models/project_spec.rb                  |
      | spec/controllers/projects_controller_spec.rb |
    When I run "rails g niftier:scaffold Task project_id:integer"
    Then I should see the following files
  ...
  Scenario: Report error when invalid model name
    Given a new Rails app
    Then I should see "Usage:" when running "rails g niftier:scaffold name:string parent_id:integer"

  Scenario: Generate scaffold for namespaced resource
    Given a new Rails app
    When I run "rails g niftier:scaffold Admin::User name:string"
    Then I should see the following files
      | app/models/user.rb                        |
    ...
    When I run "rails g niftier:layout -f"
    And I run "rake db:migrate"
    And I should successfully run "rails g niftier:scaffold Admin::User -f"
    Then I should successfully run "rake test"

  Scenario: Generate scaffold with a namespaced model
    Given a new Rails app
    When I run "rails g niftier:scaffold Admin::User name:string --namespace_model"
    ...
    When I run "rails g niftier:layout -f"
    And I run "rake db:migrate"
    And I should successfully run "rails g niftier:scaffold Admin::User -f --namespace_model"
    Then I should successfully run "rake test"

  Scenario: Given scaffold with a new and index action
    Given a new Rails app
    When I run "rails g niftier:scaffold Project name:string index new"
